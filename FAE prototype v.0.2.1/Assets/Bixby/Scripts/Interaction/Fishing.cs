...
public class Fishing : MonoBehaviour
{
    ...
    [SerializeField] protected float minGageSpeed = 0.1f; //게이지 속도
    [SerializeField] protected float maxGageSpeed = 0.5f; //게이지 속도
    [SerializeField] protected float fishingTime = 5.0f; //낚시 시간
    [SerializeField] protected float fishingPower = 0.2f; //낚시 힘

    float resultGageSpeed; //게이지 속도
    float dtime;
    ...
    IEnumerator Check_Start()
    {
        ...
        StartCoroutine(RandomGage(minGageSpeed, maxGageSpeed));
    }

    //게이지 감소 랜덤 설정
    IEnumerator RandomGage(float min, float max)
    {
        while (start)
        {
            yield return new WaitForSeconds(2.0f);
            resultGageSpeed = Random.Range(min, max);
        }
    }
