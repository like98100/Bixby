...
public class inventoryObject : MonoBehaviour
{
    ...
    public void itemExit()//인벤토리에서 아이템에 마우스를 내려뒀을 때 실행
    {
        itemSummary.SetActive(false);
    }
    public void itemSummaryMove()//itemSummary 위치 이동함수
    {
        itemSummary.transform.position = Input.mousePosition + Vector3.right * 150f;
    }
    public void itemLeftDown(itemObject itemObj)//인벤토리에서 아이템을 좌클릭 했을 때 실행
    {
